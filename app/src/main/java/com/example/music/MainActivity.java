...
public class MainActivity extends AppCompatActivity {
    ...
    static boolean playing = false; //used to know if audio is currently palying
    static boolean wasPlaying = false; //used with touching and letting go of the seekbar to tell if the audio was paused before touching it
    static boolean shuffle = false; //used to know when the shuffle list is active
    static boolean loop = false; //used to know when to loop a song
    static boolean skip = false; //used to know when skipping to the next/previous song
...
//        Log.d("File", files[audioFileIndex].getPath());
    static File[] currentFiles = files;
    static File[] shuffleFiles = files.clone();
...
//                Log.d("printing", "player:" + player);
        ...
        play_pause_btn.setOnClickListener(new View.OnClickListener(){
            public void onClick(View v) { //not sure what the "v" is for but it is a pain to take some stuff out of here and put it into other places
                if (!playing){ //starts playing audio
...
//                        player = MediaPlayer.create(v.getContext(), R.raw.long_bruh); //the og sound i used for testing
        ...
        back_btn.setOnClickListener(new View.OnClickListener(){
            public void onClick(View v) {
                player.stop();
                audioFileIndex = getNextorPriviousIndex(-1, audioFileIndex, currentFiles.length -1);
                setupAudio();
            }
        });
        ...
        shuffle_btn.setOnLongClickListener(new View.OnLongClickListener(){
            @Override
...
//            notificationChannel.setShowBadge(false);
        ...
        builder.setSmallIcon(R.mipmap.ic_launcher_raster)
                .setContentTitle(audioName.getText())
//                .setContentText("textContent2")
    ...
    public void setupAudio() {
        ...
        player = MediaPlayer.create(MainActivity.this, Uri.parse(currentFiles[audioFileIndex].getPath())); //sets up the player
        audioName.setText(currentFiles[audioFileIndex].getName().substring(0, currentFiles[audioFileIndex].getName().lastIndexOf(".")));
        builder.setContentTitle(audioName.getText());
        notificationManager.notify(1, builder.build());
        audioDuration = player.getDuration(); // gets length of audio clip
        audioDurationText.setText(getTimeLayout(audioDuration));
        getAudioPic(currentFiles[audioFileIndex].getPath());
        audioCurrentTime.setText(getTimeLayout(0));
        timeBar.setProgress(0);
        setupPlayerForComplete();
    }


    public void setupPlayerForComplete() {
...
//            if (!play) {
                ...
                Log.d("Test", "player.getCurrentPosition():" + player.getCurrentPosition() + "    timeOfAudio:" + timeOfAudio
                        + "     audioDuration:" + audioDuration + "      audioProgress:" + audioProgress + "   CurrentTime:" + System.currentTimeMillis());
            }//else {
